...
## Product state anchor (as of 2026-08-26 — verify against the board before relying on details)

R1 "ABN Morning Position" mid-flight: R1.0 sealed; R1.1 at ~87% held open pending the founder re-walk (fe#142, a blocked verification gate — last defects fe#148/fe#150); R1.5 Cloud Groundwork migrating the stack to GCP (dev estate ≈$0.12/month, europe-west4, keyless WIF, one Cloud SQL pending topology decision); prod (be#223) gated on the "full workflows" brainstorm, deliberately not yet held. Open product decisions typically tracked in the Morning Board's "waiting on you" section.
