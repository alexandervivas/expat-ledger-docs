...
- The authoritative public contract for PDF statement import — upload, import-draft retrieval and lifecycle, reconciliation outcome, confirmation, and discard — is defined in `docs/contracts/openapi/v1/openapi.yaml` under `/v1/statement-imports` (issue #79). The architectural decisions behind it are recorded in [ADR-025](/reference/backend/decisions/ADR-025-statement-import-draft-lifecycle.md).
